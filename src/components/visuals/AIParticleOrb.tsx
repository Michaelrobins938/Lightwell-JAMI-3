...
import { WebGLRenderer, ShaderMaterial, Points, Group, Vector3, PerspectiveCamera } from 'three'
...
const NarratorOrb = (() => {
  ...
  return class NarratorOrb {
    ...
    private nebulaMaterial: any;
    private tendrilMaterial: any;
    private nebulaCore: any;
    private nebulaTendrils: any;
    private orbGroup: any;
    constructor(scene: any, camera: any, renderer: any, analyserNode: AnalyserNode | null = null, config: any = {}) {
      this.scene = scene;
      this.camera = camera;
    ...
    createCoreNebula() {
      ...
      for (let i = 0; i < this.config.coreParticleCount; i++) {
        ...
        const noiseValue = this.fbm(new Vector3(
    ...
    createTendrils() {
      ...
      for (let i = 0; i < this.config.tendrilParticleCount; i++) {
        ...
        const curlNoise = this.fbm(new Vector3(x, y, z).multiplyScalar(0.5));
        x += curlNoise * 0.5;
        y += curlNoise * 0.3;
        z += curlNoise * 0.4;
...
export function AIParticleOrb({ 
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn('WebGL not supported, AI Particle Orb will not render:', error)
      setWebglError(true)
      return
    }
    ...
    const isMobile = window.innerWidth <= 768
    const mobileConfig = {
      coreParticleCount: isMobile ? Math.floor(particleCount * 0.5) : particleCount,
      tendrilParticleCount: isMobile ? Math.floor(tendrilCount * 0.5) : tendrilCount,
    ...
    try {
      ...
      console.warn('Failed to create NarratorOrb:', error)
      setWebglError(true)
      renderer.dispose()
  ...
  }, [isVisible, analyserNode, intensity, particleCount, tendrilCount])
  ...
  if (webglError) {
    return (
      <div 
        className={`ai-particle-orb-fallback ${className}`}
        style={{
          position: 'relative',
          width: '100%',
          height: '100%',
      ...
      >
        <div
          style={{
            ...
            opacity: 0.6,
            animation: 'ai-orb-pulse 2s ease-in-out infinite alternate'
          }}
        />
  ...
  return (
    <div 
      ref={containerRef} 
      className={`ai-particle-orb-container ${className}`}
      style={{
        position: 'relative',
        width: '100%',
        height: '100%',
